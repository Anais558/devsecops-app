...
function getSafeApiUrl(): string {
  ...
  try {
    ...
    const allowedBase = new URL(ALLOWED_API)

    // Validation stricte :
    // - protocole autorisé
    // - même origine que l’API autorisée
    // Cela évite la construction d’URL depuis une source non fiable
    if (
    ...
    return parsed.origin + allowedBase.pathname
  } catch {
    return ALLOWED_API
  }
}
...
async function apiFetch(url: string, options?: RequestInit): Promise<Response> {
  ...
  // Sonar préfère URL.origin plutôt que startsWith()
  if (
    ...
    <div style={styles.body}>
      <div style={styles.app}>
        ...
        <div style={styles.header}>
          <h1 style={styles.h1}>
            Mes <em style={styles.em}>tâches</em>
          </h1>
          <p style={styles.subtitle}>
            DevSecOps Project · Todo App
          </p>
        </div>
          ...
          {todos.map(t => (
            ...
            >
              <button
                style={{
                  ...styles.checkBtn,
                  background: t.done ? '#c8f060' : 'transparent',
                  borderColor: t.done ? '#c8f060' : '#3e3d38'
                }}
